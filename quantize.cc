...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteFloat32) {
    ...
    // Int8 to quantized output conversion
    size_t size = ElementCount(*input->dims);
    switch (output->type) {
      case kTfLiteInt8:
      ...
      case kTfLiteUInt8:
        ...
        for (size_t i = 0; i < size; i++) {
          tflite::micro::GetTensorData<uint8_t>(output)[i] = 
              (uint8_t)(tflite::micro::GetTensorData<int8_t>(input)[i] + 128);
        }
        break;
      case kTfLiteInt16:
        // Convert int8 to int16
        for (size_t i = 0; i < size; i++) {
          tflite::micro::GetTensorData<int16_t>(output)[i] = 
              (int16_t)tflite::micro::GetTensorData<int8_t>(input)[i];
        }
        break;
      default:
        TF_LITE_KERNEL_LOG(context, "Input %s, output %s not supported.",
    ...
    // UInt8 to quantized output conversion
    size_t size = ElementCount(*input->dims);
    switch (output->type) {
      case kTfLiteUInt8:
        // Direct copy for same type
        memcpy(tflite::micro::GetTensorData<uint8_t>(output),
      ...
      case kTfLiteInt8:
        ...
        for (size_t i = 0; i < size; i++) {
          tflite::micro::GetTensorData<int8_t>(output)[i] = 
              (int8_t)(tflite::micro::GetTensorData<uint8_t>(input)[i] - 128);
        }
        break;
      case kTfLiteInt16:
        ...
        for (size_t i = 0; i < size; i++) {
          tflite::micro::GetTensorData<int16_t>(output)[i] = 
              (int16_t)(tflite::micro::GetTensorData<uint8_t>(input)[i] - 128);
        }
        break;
      default:
        TF_LITE_KERNEL_LOG(context, "Input %s, output %s not supported.",
